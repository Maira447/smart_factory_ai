...
st.set_page_config(
    page_title="FactoryMind AI | Smart Dashboard",
    page_icon="🏭",
    layout="wide",
...
def login_signup_page():
    st.markdown('<h1 class="glow-text">FactoryMind AI Portal</h1>', unsafe_allow_html=True)
...
def top_right_controls():
    ...
    col_space, col1, col2, col3 = st.columns([8,1,1,1])
    ...
    with col2:
        if st.button("🎛 Control", key="top_control"):
...
with st.sidebar:
    st.markdown(
        '<h2 style="color:white; margin-bottom:20px;">FactoryMind'
        ...
        if uploaded_file and d_model:
            ...
            if GROQ_API_KEY:
                with st.spinner("Expert Analysis in progress..."):
                    ...
                    st.markdown("### 🧠 AI Expert Insights")
                    st.write("Suggested Corrective Action: Adjust needle tension on Loom #4 and inspect yarn batch for contamination.")
                    st.markdown('</div>', unsafe_allow_html=True)
            
            log_to_csv("defect_log.csv", {"class": class_name, "confidence": conf, "time": pd.Timestamp.now()})
...
# --- LSTM FORECASTING ---
    ...
    data = pd.DataFrame({
        'Actual': np.random.randint(100, 200, 12),
        'Forecast': [None]*8 + list(np.random.randint(150, 220, 4))
    })
    st.line_chart(data)
    st.info("The LSTM neural network predicts a 12% increase in production demand for Q3.")
    st.markdown('</div>', unsafe_allow_html=True)

# Footer
st.markdown("<br><hr><p style='text-align: center; color: #94A3B8;'>Smart Factory Intelligence Platform | Powered by Llama 3 & Groq AI</p>", unsafe_allow_html=True)